...
const CONTRIBUTION_COLORS = [
  ...
  "bg-amber-200", // Level 1
  "bg-amber-300", // Level 2
  "bg-amber-400", // Level 3
    ...
    <div className={`contribution-graph ${className} mt-17 mb-27 `}>
      ...
      {showTooltips && hoveredDay && (
        <motion.div
          ...
          className="pointer-events-none fixed z-50 rounded-lg border bg-muted px-3 py-2 text-foreground text-sm shadow-lg"
      ...
      {/* Legend */}
      {showLegend && (
        <div className="mt-4 flex items-center justify-center gap-3 text-foreground/70 text-xs">
          <span>Less</span>
          <div className="flex items-center gap-1">
            {CONTRIBUTION_LEVELS.map((level) => (
              <div
                className={`h-3 w-3 rounded-[3px] ${CONTRIBUTION_COLORS[level]}`}
                key={level}
              />
            ))}
          </div>
